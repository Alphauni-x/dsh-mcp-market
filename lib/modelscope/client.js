 ...
 * 取一个真正能当输入提示的示例值。
 *
 * 广场把 `test_value` 当作「拿它去跑连通性测试」的值，而不是文档示例：FireCrawl
 * 这类服务的 5 个环境变量 `test_value` 全是 `1`。照搬成 placeholder 会让人以为
 * 输入框里已经预填了内容，所以只保留有信息量的值，其余一律留空。
 *
 * @param {object} spec 单个属性的原始描述。
 * @returns {string} 可用的示例，或空串。
 */
function exampleValueOf(spec) {
  const raw = spec?.test_value;
  if (raw === undefined || raw === null) return "";
  const text = String(raw).trim();
  if (text.length < 3) return "";
  if (/^[+-]?\d+(\.\d+)?$/.test(text)) return "";
  if (/^(true|false|null|undefined|none)$/i.test(text)) return "";
  return text;
}
