...
import { fiberPhaseOf, getLoaderEntry, mcpToolCount, mcpToolNames, waitForLoaderState } from "./status.js";
...
export class InstallerGateway extends TypertRemoteService {
   ...
   *
   * 已安装的行分两条路走，区别只在「宿主里有没有一份活着的连接」：
   *
   *   - **已经跑起来**（fiber active 且确实注册了工具）→ 直接读宿主，微秒级返回。
   *     宿主里那份连接本来就是真的，另起一个进程只是把同样的启动成本再付一遍
   *     （实测 stdio 每次要重建运行环境，545ms 里 450ms 花在这儿）。
   *   - **其它的**（没装、停用、或装了但一个工具都没注册）→ 老老实实真连一次，
   *     因为「为什么起不来」只有真连能给出答案。
   *
   * @param {object} payload
   */
  async test(payload = {}) {
    if (typeof payload.serverName === "string" && payload.transport === undefined) {
      ...
      const live = this.liveToolNames(row, config.serverName);
      if (live !== undefined) {
        return { ok: true, tools: live.map((name) => ({ name })), source: "host" };
      }
  ...
  /**
   * 宿主里这份连接已经活着了吗？活着就返回已注册的工具名（已剥前缀）。
   *
   * 三个条件都满足才算命中，缺一不可：
   *   1. 行有 id 且能查到 loader 条目；
   *   2. 条目处于 `active`（`failed` / `pending` 都不算）；
   *   3. 至少注册了 1 个工具 —— 这一条同时排除了「绿点但在撒谎」的情形：
   *      stdio 起不来时插件 fiber 照样 active（failOnStartupError 默认 false），
   *      只有工具数会诚实地停在 0。这种必须走真连才能拿到失败原因。
   *
   * @param {object} row patch 行。
   * @param {string} serverName
   * @returns {string[]|undefined} 命中返回工具名，否则 `undefined`。
   */
  liveToolNames(row, serverName) {
    const entry = typeof row?.id === "string" ? getLoaderEntry(this.C, row.id) : undefined;
    if (fiberPhaseOf(entry?.fiber?.state) !== "active") return undefined;
    const names = mcpToolNames(this.C, serverName);
