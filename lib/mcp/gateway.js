...
export class InstallerGateway extends TypertRemoteService {
  ...
  async installServer(payload = {}) {
    ...
    const check = validateServerInput(plan.config);
    if (check.ok !== true) throw new Error(check.error);

    const { managed, external } = await this.readRows();

    for (const row of external) {
      if (configFromPatchRow(row)?.serverName === plan.serverName) {
        throw new Error(
          `serverName "${plan.serverName}" 已被 cordis.patch.yml 中的外部 MCP 行占用，请先在文件中处理`,
        );
      }
    }
    if (managed.some((row) => configFromPatchRow(row)?.serverName === plan.serverName)) {
      throw new Error(`"${plan.serverName}" 已安装过了`);
    }

    const enabled = payload.enabled !== false;
    const row = toPatchRow(plan.config, enabled);
