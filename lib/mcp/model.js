 ...
 * 把手动添加表单的输入整理成官方配置形态。
 *
 * 只保留用户确实填了的键：空串、空数组、空键值对一律省略，免得配置文件里
 * 出现 `cwd: ""` 这类无意义字段。
 *
 * 两种 transport 的字段是互斥的，且必须严格贴住 `dsh-mcp-client` 的 schema：
 * `StdioConfig` 才有 `command / args / env / cwd`，`StreamableHttpConfig` 才有
 * `url / headers`。给远程配置塞 `env` 不会报错，但会被 schema 静默丢掉 ——
 * 也就是填了等于没填，所以这里不生成它。
 *
 * 产物仍须过一遍 `validateServerInput`：这里只负责整形，不负责判合法性。
 *
 * @param {object} input 前端表单的原始输入。
 * @returns {object} 官方形态的配置。
 */
export function configFromCustomInput(input) {
  const source = input && typeof input === "object" ? input : {};
  const serverName = typeof source.serverName === "string" ? source.serverName.trim() : "";
  ...
  if (source.transport === "stdio") {
    ...
    const env = cleanStringMap(source.env);
  ...
  const headers = cleanStringMap(source.headers);
