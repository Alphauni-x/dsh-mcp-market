  ...
  factory: (require) => {
      ...
      return h(
        ...
        confirming
          ...
          : h(
              ...
              h(Button, { variant: "ghost", size: "sm", disabled: busy, onClick: () => onTest(item) }, testState?.state === "running" ? t("testing") : t("test")),
              h("span", { className: "MM_spacer" }),
              item.managed
    ...
    const MarketPage = (props) => {
      ...
      const test = async (item) => {
        setTestState((prev) => ({ ...prev, [item.serverName]: { state: "running" } }));
        try {
          ...
          setTestState((prev) => ({
            ...
            [item.serverName]: result?.ok
              ? { state: "ok", text: t("testOk", { n: tools.length }), names }
