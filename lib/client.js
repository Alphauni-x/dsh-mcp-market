...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      serverName: "服务标识",
      serverNameHint: "该名称作为工具的命名空间前缀，仅支持字母、数字、下划线与连字符。",
      command: "启动命令",
      args: "启动参数",
      url: "服务地址",
      envTitle: "环境变量",
      ...
      removeFail: "删除失败",
      test: "测试连接",
      testingElapsed: "测试中… {s}s",
      testingSlow: "本地服务首次运行需要下载依赖，可能要好几分钟；安装会在后台继续，稍后重试即可。",
      testOk: "连接成功，服务返回 {n} 个工具。",
      testOkHost: "运行中的实例已注册 {n} 个工具。",
      testFail: "连接失败",
      testTools: "工具：",
      testToolsMore: "等 {n} 个",
      listSep: "、",
    ...
    const en = {
      ...
      serverNameHint: "Namespace prefix for this server's tools. Letters, digits, underscores and hyphens only.",
      command: "Command",
      args: "Arguments",
      url: "Endpoint",
      ...
      test: "Test",
      testingElapsed: "Testing… {s}s",
      testingSlow: "A local server's first run downloads its dependencies, which can take minutes. The install keeps going in the background — just test again shortly.",
      testOk: "Connected — the server exposes {n} tools.",
      testOkHost: "The running instance exposes {n} tools.",
      testFail: "Connection failed",
      testTools: "Tools: ",
      testToolsMore: "and {n} more",
      listSep: ", ",
      enable: "Enable",
    ...
    const CSS = [
      ...
      ".MM_err{font-size:12px;line-height:18px;color:var(--dsw-alias-state-error-primary);margin:0;word-break:break-word;white-space:pre-wrap}",
     ...
     *
     * `testState` 统一成 `{ state, text?, names? }` 这一种形状 —— 早先「测试中」
     * 存的是裸字符串 `"running"`，结果态存的是对象，渲染时用 `=== "ok"` 去比
     * 对象，成功/失败提示永远不显示。
     */
    const TOOL_NAME_CAP = 8;

    /** 工具名列表：超出上限就截断，免得一个 30 工具的服务把卡片撑爆。 */
    const toolNamesText = (names, t) => {
      const shown = names.slice(0, TOOL_NAME_CAP);
      const rest = names.length - shown.length;
      const text = shown.join(t("listSep"));
      return rest > 0 ? `${text} ${t("testToolsMore", { n: rest })}` : text;
    };

    /**
     * 测试已进行的秒数，保留一位小数。
     *
     * 时钟在父组件里（只在真有测试在跑时才走），这里只做换算，卡片保持纯函数。
     */
    const elapsedSeconds = (state, now) => {
      const startedAt = state?.startedAt;
      if (typeof startedAt !== "number" || typeof now !== "number" || now <= 0) return "0.0";
      return Math.max(0, (now - startedAt) / 1000).toFixed(1);
    };

    /** 超过这个秒数还没回来，就提示「多半在下载依赖」。 */
    const SLOW_TEST_MS = 5000;

    /**
     * 测试是不是已经慢到值得解释了。
     *
     * 一次真的 stdio 探测里，几百毫秒是常态；超过 5 秒还静悄悄，最可能的原因就是
     * 包运行器正在首次下载依赖 —— 而这恰好是用户最容易误判成「卡死了」的时候。
     */
    const isSlowTest = (state, now) =>
      state?.state === "running" &&
      typeof state.startedAt === "number" &&
      typeof now === "number" &&
      now - state.startedAt > SLOW_TEST_MS;

    const InstalledCard = ({ item, t, busy, confirming, onToggle, onAskRemove, onRemove, onCancelRemove, onTest, testState, now }) => {
      ...
      return h(
          ...
          : h(
              ...
              h(
                Button,
                {
                  variant: "ghost",
                  size: "sm",
                  // 测试期间必须禁用：连点会为同一个服务再起一个进程，两个包运行器
                  // 同时往同一份缓存里写，只会更慢、也更容易留下半成品。
                  disabled: busy || testState?.state === "running",
                  onClick: () => onTest(item),
                },
                testState?.state === "running"
                  ? t("testingElapsed", { s: elapsedSeconds(testState, now) })
                  : t("test"),
              ),
              h("span", { className: "MM_spacer" }),
              item.managed
        ...
        isSlowTest(testState, now) ? h("p", { className: "MM_note" }, t("testingSlow")) : null,
        testState?.state === "ok" ? h("p", { className: "MM_ok", role: "status" }, testState.text) : null,
        testState?.state === "ok" && testState.names?.length
          ? h(
              "p",
              { className: "MM_cardMeta", title: testState.names.join(t("listSep")) },
              `${t("testTools")}${toolNamesText(testState.names, t)}`,
            )
          : null,
        testState?.state === "fail" ? h("p", { className: "MM_err", role: "status" }, testState.text) : null,
      );
    };
        ...
        h(
          ...
          h(
            ...
            h(
              ...
              h("p", { className: "MM_note" }, t("serverNameHint")),
            ),
    ...
    const MarketPage = (props) => {
       ...
       * 测试连接的走秒时钟。
       *
       * 只在真有测试在跑时才启，跑完立刻停 —— 空转一个 setInterval 会让整个面板
       * 每 200ms 重渲染一次。stdio 型首次连接可能要好几秒（包运行器要重建运行环境），
       * 让用户看见秒数在动，比一个静止的「测试中…」好等得多。
       */
      const [clock, setClock] = useState(0);
      const anyTesting = Object.values(testState).some((state) => state?.state === "running");
      useEffect(() => {
        if (!anyTesting) return undefined;
        setClock(Date.now());
      ...
      const test = async (item) => {
        setTestState((prev) => ({ ...prev, [item.serverName]: { state: "running", startedAt: Date.now() } }));
        try {
          const result = await props.api.test({ serverName: item.serverName });
          const tools = Array.isArray(result?.tools) ? result.tools : [];
          const names = tools.map((tool) => (typeof tool?.name === "string" ? tool.name : "")).filter((name) => name !== "");
          setTestState((prev) => ({
            ...prev,
            [item.serverName]: result?.ok
              ? {
                  // 已装载的服务走的是宿主里那份活连接，不是新起进程 —— 说清楚，
                  // 免得用户以为「秒回」是没真测。
                  state: "ok",
                  text: t(result?.source === "host" ? "testOkHost" : "testOk", { n: tools.length }),
                  names,
                }
