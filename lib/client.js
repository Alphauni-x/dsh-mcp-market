  ...
  factory: (require) => {
    ...
    const CustomDialog = ({ t, submitting, error, onCancel, onSubmit }) => {
      ...
      const patchRow = (index, patch) =>
        setEnvRows((rows) => rows.map((row, at) => (at === index ? { ...row, ...patch } : row)));
        ...
        h(
          ...
          h(
            ...
            h(
              "div",
              { className: "MM_field" },
              h("span", { className: "MM_label" }, `${t("envTitle")} · ${t("envOptional")}`),
              ...envRows.map((row, index) =>
                h(
                  "div",
                  { className: "MM_kvRow", key: `env-${index}` },
                  h("input", {
                    className: "MM_input",
                    value: row.key,
